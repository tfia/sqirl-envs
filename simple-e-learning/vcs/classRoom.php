...
if(mysqli_num_rows($user_details_query) <= 0){
    echo "<script>alert('Class Code is Unknown or has been deleted.'); location.replace(document.referrer)</script>";
}
...
if (isset($_POST['post'])) {
    ...
    $new_post = $post->submitPost($_POST['post_text'], 'none', 'none', $teacherName);
		...
		$(function(){
			$('.comment_iframe').on("load", function(){
                if($(this).parent().css('display') != 'none'){
                    ...
                    var comments = $($(this)[0].contentWindow.document).find('.comment_section').length
                    console.log(comments,'#comment'+id, $('#comment'+id)[0], `Comment ${(comments) > 0 ? 's' :''}(${comments})`)
                    $('#comment'+id).text(`Comment${(comments) > 0 ? 's' :''}(${comments})` )
                }
            })
		})	
	</script>
	...
	$(function(){
		...
		if(hash != ''){
			...
			Object.keys(hash).map(k=>{
				var data = hash[k].split("=")
				var key = data[0]
				var val = data[1]
				window[key] = val
			})
			// Has Post ID
			if(typeof window['post_id'] != 'undefined' > 0 && typeof window['comment'] == 'undefined'){
				if($(`.status_post[data-id="${post_id}"]`).length > 0){
                    $('html, body').scrollTop($(`.status_post[data-id="${post_id}"]`).offset().top)
                }
                $(`.status_post[data-id="${post_id}"]`).addClass("focus")
                setTimeout(() => {
                    $(`.status_post[data-id="${post_id}"]`).removeClass("focus")
                }, 1500);
			}
			...
			if(typeof window['comment'] != 'undefined' ){
                ...
                $(`#toggleComment${post_id}`).show()
                setTimeout(() => {
                    $('html, body').scrollTop($(`iframe.comment_iframe[data-id="${post_id}"]`).offset().top+500)

                    var comment_frame = $($(`iframe.comment_iframe[data-id="${post_id}"]`)[0].contentWindow.document)
                    samp = comment_frame;
                    setTimeout(() => {
                        console.log(comment_frame.find(`.comment_section[data-id='${comment}']`).offset().top)
                        comment_frame.scrollTop(comment_frame.find(`.comment_section[data-id='${comment}']`).offset().top)
                        comment_frame.find(`.comment_section[data-id='${comment}']`).addClass('focus')
                        setTimeout(() => {
                            comment_frame.find(`.comment_section[data-id='${comment}']`).removeClass('focus')
                        }, 1300);
                    }, 500);
                }, 500);
			}
